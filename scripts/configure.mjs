...
try {
  ...
  if(answer){const incoming=readJSON(path.resolve(answer)),draftFile=path.join(ROOT,'.argus-onboarding.json'),draft=readJSON(draftFile,{answers:{}});const allowed=['purpose','preset','goals','sources','panels','name','timezone','branding','voice','provider','vault'];const answers={...draft.answers};for(const k of allowed)if(k in incoming)answers[k]=incoming[k];atomicJSON(draftFile,{stage:'interview',answers,updatedAt:new Date().toISOString()});console.log('Interview progress saved.');process.exit(0);}
  ...
  const old=loadConfig(),next={...old,...incoming,voice:{...old.voice,...incoming.voice}};
  ...
  if(old.workspace!==clean.workspace&&fs.existsSync(workspace(old))&&fs.readdirSync(workspace(old)).length)throw new Error('This workspace already contains data. Export or copy it deliberately before changing workspace; no files were moved.');
  ...
  if(clean.onboarded)atomicJSON(path.join(ROOT,'.argus-onboarding.json'),{stage:'complete',completedAt:new Date().toISOString()});
