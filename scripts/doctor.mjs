...
import {readVoiceInstall} from '../runtime/voice-install.mjs';
try {
  ...
  const voice=readVoiceInstall();console.log(`Voice installation: ${voice.status} — ${voice.message}`);console.log(`Speaking: ${voice.speaking?'verified':'not ready'} · Listening: ${voice.listening?'verified':'not ready'}`);if(voice.serviceError)console.log(`Voice service: ${voice.serviceError}`);
