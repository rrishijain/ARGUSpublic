...
import {startVoiceInstall,ensureVoiceService,stopVoiceService} from '../runtime/voice-install.mjs';
...
let voiceTimer;
const stop=()=>{if(stopping)return;stopping=true;if(voiceTimer)clearInterval(voiceTimer);stopVoiceService();for(const child of children){if(process.platform==='win32')spawnSync('taskkill',['/pid',String(child.pid),'/t','/f'],{windowsHide:true});else child.kill('SIGTERM');}server.close();fs.rmSync(sessionFile,{force:true});setTimeout(()=>process.exit(0),1500);};
...
const reconcileVoice=async()=>{if(stopping||voiceChecking)return;voiceChecking=true;try{await ensureVoiceService();}catch(error){console.log(`Voice: ${error.message}`);}finally{voiceChecking=false;}};
void reconcileVoice();voiceTimer=setInterval(reconcileVoice,3000);
