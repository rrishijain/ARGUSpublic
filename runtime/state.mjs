...
export function getState(root=ROOT) {
  ...
  return {config:publicConfig(c),sources:sourcesFor(dir).map(({id,name,status,capturedAt,columns,rowCount})=>({id,name,status,capturedAt,columns,rowCount,stale:Date.now()-Date.parse(capturedAt)>86400000})),tasks:readJSON(path.join(dir,'system/tasks.json'),[]),notes:readJSON(path.join(dir,'system/notes.json'),[]),jobs:jobsFor(dir).slice(0,30),metrics:c.metrics.map(m=>({...m,...metricValue(dir,m)})),providers:available,runner,workflows:WORKFLOWS.map(({instruction,...w})=>({...w,enabled:Boolean(c.onboarded&&c.provider&&available[c.provider]?.installed&&runner.alive),reason:!c.onboarded?'Complete the interview first.':!c.provider?'Select your AI provider.':!available[c.provider]?.installed?'Install and sign in to the selected CLI.':!runner.alive?'Start the runner with npm start.':null}))};
}
