...
On 11 September 2026, a clean local managed runtime and real synthesis/recognition round trip passed on Apple Silicon. Hosted macOS arm64, Windows x64 and Ubuntu x64 passed fresh and repeated installation plus real inference; see the exact [platform matrix](../docs/validation.md). Binary-only lock resolution also passed for Intel macOS and Linux x64/glibc 2.28. These checks do not establish successful device-level execution on every baseline. Recognition of Indian accents requires participant pilot recordings; synthesised test speech is not evidence of accent accuracy.
