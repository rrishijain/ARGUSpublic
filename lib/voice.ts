...
export class Voice {
  ...
  stop(broadcast=true){this.generation++;this.complete?.();this.complete=null;this.abort?.abort();this.abort=null;this.audio?.pause();this.audio=null;if(this.objectURL)URL.revokeObjectURL(this.objectURL);this.objectURL=null;window.speechSynthesis?.cancel();this.onSpeaking(false);if(this.beat)clearInterval(this.beat);this.beat=null;this.context?.close().catch(()=>{});this.context=null;this.analyser=null;if(broadcast)try{localStorage.setItem('argus.student.stop',String(Date.now()));}catch{}}
  ...
  async speak(text:string,ambient=false){
    ...
    // Background report notifications must not take over an active conversation.
    try{const microphone=JSON.parse(localStorage.getItem('argus.conversation.microphone')||'null');if(ambient&&microphone&&Date.now()-microphone.time<5000)return;}catch{}
    ...
    let resolveCompletion:()=>void=()=>{};const completion=new Promise<void>(resolve=>{resolveCompletion=resolve;});this.complete=resolveCompletion;
    const finish=()=>{resolveCompletion();if(generation!==this.generation)return;this.complete=null;this.onSpeaking(false);if(this.beat)clearInterval(this.beat);this.beat=null;this.context?.close().catch(()=>{});this.context=null;this.analyser=null;if(this.objectURL)URL.revokeObjectURL(this.objectURL);this.objectURL=null;this.audio=null;};
    ...
    if(this.settings.engine==='kokoro'){
      try {
        ...
        a.onended=finish;a.onerror=()=>{finish();this.onNotice('Playback failed. Your reply is shown as text.');};await a.play();if(generation!==this.generation)return;this.onSpeaking(true);await completion;return;
    ...
    u.onstart=()=>this.onSpeaking(true);u.onend=finish;u.onerror=()=>{finish();this.onNotice('Tap Preview voice to enable speech, or continue using text.');};window.speechSynthesis.speak(u);await completion;
  }
}
